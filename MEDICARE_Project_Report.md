...
```
                    ...
                    │                                         │
  ┌──────────┐  ───▶│                                 ───▶  ┌──────────────┐
  │ PATIENT  │      │       MEDICARE HOSPITAL         │     │   CLOUDINARY │
...
```
Patient ──(1)──▶ [1.0 Authenticate User] ──(2)──▶ Clerk Auth Service
                         │
                    (3) Token
                         │
                         ▼
Patient ──(4)──▶ [2.0 Select Doctor] ──(5)──▶ {Doctors DB}
                         │
                    (6) Doctor Data
                         │
                         ▼
Patient ──(7)──▶ [3.0 Book Appointment] ──(8)──▶ {Appointments DB}
                         │
                    (9) Validate Slot
                         │
                         ▼
                  [4.0 Check Duplicate] ──(10)──▶ {Appointments DB}
                         │
                    (11) Slot Available
                         │
                         ▼
                  [5.0 Create Record] ──(12)──▶ {Appointments DB}
                         │
                    (13) Trigger Notification
                         │
                         ▼
                  [6.0 Send Email] ──(14)──▶ Nodemailer / SMTP
                         │
                    (15) Confirmation
                         │
                         ▼
                     Patient ◀── (16) Success Response
```

### 5.3.3 Level 1 DFD — Admin Operations

```
Admin ──(1)──▶ [1.0 Clerk Authentication]
                       │
                  (2) Verified
                       │
         ┌─────────────┴──────────────────────┐
         │             │                      │
         ▼             ▼                      ▼
[2.0 Manage      [3.0 Manage           [4.0 View
  Doctors]         Services]           Dashboard]
    │                 │                     │
    ▼                 ▼                     ▼
...
## 10.1 System Output Description
